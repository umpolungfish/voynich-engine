...
The other folios in this manuscript admit interpretations that require some imagination: one can argue the botanical stems resemble FSPLIT chains, or that the cosmological rosette resembles a lattice traversal. The f116r correspondence requires no argument. Both panels of Figure~\ref{fig:compare_f116r} are cursive.

\begin{figure}[htbp]
\centering
\begin{minipage}[t]{0.47\textwidth}
  \centering
  \includegraphics[width=\textwidth]{folio_images/f116r.jpg}
  \caption*{\textit{f116r} (Beinecke MS 408, CC BY-SA 4.0). The final text folio. Flowing cursive script with tight loops, crossings, and recurved strokes.}
\end{minipage}
\hfill
\begin{minipage}[t]{0.47\textwidth}
  \centering
...
\end{minipage}
\caption{f116r: manuscript folio vs.\ engine call graph. The call graph layout algorithm and the hand that wrote the folio arrived at the same shape from the same sequential structure. The spring embedding did not know what the glyphs look like.}
